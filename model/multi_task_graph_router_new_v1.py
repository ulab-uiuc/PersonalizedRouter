...
import numpy as np
import torch
from graph_nn_new_v1 import  form_data,GNN_prediction
...
class graph_router_prediction:
    ...
    def split_data(self):
        ...
        for i, (preference, effect, cost) in enumerate(zip(self.preference_list, self.effect_list, self.cost_list)):
        ...
        optim_indeices = np.split(optim_index, self.num_users, axis=0)
        label_list = []
        for i in range(self.num_users):
            eye_3ds[i] = np.squeeze(eye_3ds[i], axis=0)
            optim_indeices[i] = np.squeeze(optim_indeices[i], axis=0)
            label_i = eye_3ds[i][optim_indeices[i]]
    ...
    def prepare_data_for_GNN(self):
        ...
        self.best_llm_list=np.array(self.data_df['best_llm'].tolist())
        self.preference_list = np.array(self.data_df['performance_preference'].tolist())
