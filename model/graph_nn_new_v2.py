...
class form_data:
    ...
    def formulation(self,task_id,query_feature,llm_feature,user_feature,org_node,des_node,edge_feature,label,edge_mask,combined_edge,train_mask,valide_mask,test_mask,llm_mapping,cost_list,answer_list,config):
        ...
        combined_edge = torch.tensor(answer_list, dtype=torch.float).reshape(-1,1).to(self.device)
        data = Data(task_id=task_id,query_features=query_features, llm_features=llm_features, user_features=user_features, edge_index=edge_index,
                        edge_attr=edge_weight,query_indices=query_indices, llm_indices=llm_indices,label=torch.tensor(label, dtype=torch.float, device=self.device),edge_mask=edge_mask,combined_edge=combined_edge,
                    train_mask=train_mask,valide_mask=valide_mask,test_mask=test_mask,cost_list=cost_list,answer_list=answer_list)
...
class GNN_prediction:
    ...
    def train_validate(self,data,data_validate,data_for_test):
        ...
        for epoch in range(self.config['train_epoch']):
            ...
            with torch.no_grad():
                ...
                test_result,test_loss,accuracy=self.test(data_for_test,self.config['model_path'])
                self.wandb.log({"train_loss":loss_mean,"validate_loss": loss_validate,"test_loss":test_loss, "validate_accuracy": validate_accuracy,"validate_f1": f1, "test_result": test_result, "accuracy":accuracy})
    ...
    def test(self,data,model_path):
        ...
        answer_label = torch.tensor(data.answer_list[mask].reshape(-1, self.config['llm_num'])).to(edge_predict.device)
        answer_idx = torch.argmax(answer_label, 1)
        accuracy = ( max_idx == answer_idx).float().mean()
        ...
        cost_idx = data.cost_list[mask].reshape(-1, self.config['llm_num'])
        ...
        return result,loss_test,accuracy